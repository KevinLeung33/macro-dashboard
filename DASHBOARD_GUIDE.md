...
- 首页：宏观分析工作台入口。默认先看当前环境、跨资产温度计、数据质量、短中长期文字总结、变化与催化剂，再进入研究假设或 Crypto 交易工作台。
- 货币政策：联邦基金利率、美债收益率、实际利率、通胀预期。
- 市场数据：美股指数、QQQ/MAGS、TLT、HYG/LQD、DXY、美元/日元、美元/人民币、汇率和大宗商品。
...
- 交易复盘：Crypto 交易工作台；展示跨币种保证金账户、持仓、订单、成交和 K 线，并在顶部显示宏观与跨资产交易上下文。交易计划会记录入场价、挂单类型、触发价、计划数量和状态，并可关联已同步订单；支持保存计划当时的环境快照、可选 AI 计划反馈、独立 AI 影子计划与事后点评。
...
- `news`：官方/媒体 RSS（含美联储、SEC、EIA、国家统计局、ECB、吴说、Odaily）；快讯和官方发布走低延迟通道，文章和综合媒体走完整新闻任务。新闻雷达按“重要快讯 / 事件流 / AI分析 / 原始文章”展示。Odaily文章与快讯可分别通过 `ODAILY_RSS_ENABLED`、`ODAILY_NEWSFLASH_RSS_ENABLED` 关闭。官方源是健康告警的基础；吴说、CoinDesk 等第三方媒体源为可降级补充，失败会保留状态但默认不触发 P1/P2 推送。Alpha Vantage 新闻默认关闭，避免免费 Key 的 25 次/日额度被定时任务耗尽。BLS、Reuters 和 The Block 如在服务器被 403/404 拦截，会从正式订阅中退役而保留历史状态；这不影响 FRED 宏观数据。
...
首页的“当前判断摘要”分成三个阅读层次；各主题页的状态总览也提供对应的短期/中期/长期阅读提示：
...
首页只展示有限的状态摘要和变化，不为每张图单独生成结论，避免读者在几十条结论中失去重点。首页新增宏观文字总结：优先拼接当前规则与 AI 简报；主题页使用稳定的主题阅读框架，不在每次打开页面时额外调用 AI。完整图表集中在各主题页的“详细数据”标签中；每个主题页还提供“事件与证据”入口。每个摘要都会尽量同时展示数据日期、来源和质量状态。
...
- 当前宏观与市场环境。
- 风险资产、利率、信用、美元、中国资产和 BTC/ETH 的跨资产温度计。
- 摘要使用的数据质量和新鲜度。
- 近期变化、未来两周人工研究事件和重要新闻事件流。
- 活跃研究假设、观察项和 Crypto 交易计划入口。

它的目标是先回答“今天应该看哪里”，下面的详细页面再回答“为什么”。首页不再承担完整图表浏览职责；货币政策、市场数据、全球市场、Crypto、信用、就业、历史和流动性页面统一提供“状态总览 / 详细数据 / 事件与证据”三个标签，图表默认收纳在详细数据标签。

每个状态摘要会尽量显示：最新值、近几期变化、数据日期、来源和数据质量。宏观低频数据应优先看本次值、前值、发布日期和趋势，不把月度数据误读成实时行情。首页的跨资产温度计目前覆盖风险资产、利率、信用、美元、人民币、日元、大宗、中国资产和 BTC/ETH；新标的尚未成功抓取时显示暂无数据，不会用代理值伪造结论。
...
Odaily RSS 使用官方维护的文章订阅地址 `https://rss.odaily.news/rss/post`，默认作为 Crypto 新闻补充源。若源站限流或噪声过大，可在 `.env` 设置 `ODAILY_RSS_ENABLED=false`；它不会作为宏观核心数据健康依赖。
...
服务器会每 5 分钟执行一次 P0/P1/P2 健康检查：P0 检查本机 Streamlit、FastAPI 和 cpolar 公网 URL；P1 检查定时任务新鲜度、数据源/RSS 状态、SQLite 完整性和磁盘空间；P2 检查数据库备份新鲜度与完整性、新闻 AI 失败堆积和通知渠道投递状态。cpolar 默认连续 3 次检查失败才升级，连续 2 次成功才恢复，单次网络抖动只记录不推送。已停用的备用源和未配置的可选源不会升级成严重告警；只有 `HEALTH_CRITICAL_DATA_SOURCES` 中、且没有新鲜有效数据的源才会升级。健康状态会写入运行目录，服务重启后同一问题不会重新推送；状态变化或恢复时才发送告警。
...
- 快讯推送状态改为 `pending -> sending -> sent/failed`，只有通知成功后才标记 `sent`。
- 在看板中记录已推送快讯的人工反馈：重要、噪音、不确定及噪音类别。
- 每日对未推送快讯做 AI 分层，仅抽取高疑似重要、低置信度、各类别代表样本，并保留随机保底样本。
- 在看板中展示漏报候选、误报统计、来源表现和关键词表现。
- 每周生成候选规则调整报告，由人工确认后发布新规则；不允许单条反馈自动修改生产规则。

### 12.2 性能优化待办

- 首页只读取预计算的 cockpit 快照，避免打开页面时重复执行多组聚合查询。
- 将首页 AI 简报改为后台任务生成，页面只读取最近一次成功结果。
- 对跨资产行情、新闻聚类、数据质量和研究上下文增加进程缓存或数据库快照。
- 检查新闻、快讯、AI 分析相关查询的复合索引和分页，避免全表扫描。
- 将详细图表和大列表放入折叠区域或独立页面，首页只保留摘要。
- 对 Streamlit 页面进行加载耗时埋点，分别记录数据库、市场数据、新闻、AI 和渲染耗时。
- 检查刷新任务与页面查询之间的 SQLite 锁等待，必要时拆分读库和写库、减少长事务。

### 12.3 日报：过去 24 小时新闻摘要

待实现，作为日报固定模块，不与首页总览混用：

- 以日报生成时间为截止点，严格筛选过去 24 小时内已成功采集的新闻。
- 先按事件聚类和来源去重，再生成新闻摘要，避免逐条罗列重复文章。
- 区分“已确认事件”“重要但待确认”“市场噪音/低影响信息”。
- 每条摘要保留来源数量、代表性来源、最早发布时间、最近更新时间和相关资产。
- 摘要中明确区分事实、市场影响和后续观察项，不把新闻直接写成交易结论。
- 没有足够新闻时明确显示“过去 24 小时没有形成高置信度主线”，不要用长期新闻总览替代。
- 新闻摘要由后台日报任务生成并写入数据库，日报页面只读取最近一次成功结果。
- 保存摘要使用的时间窗口、新闻 ID、聚类版本和 prompt 版本，便于回溯与复盘。

### 12.4 性能架构目标

- 宏观数据按数据频率后台刷新：低频数据以日更或发布驱动为主，页面不主动抓取。
- 首页 AI 简报每天生成一次并持久化；新闻雷达、交易复盘和用户明确触发的操作保留按需 AI。
- 首页“刷新数据”改为提交后台任务，页面立即返回任务状态，不同步等待抓取和 AI。
- 首页改为读取宏观、新闻和交易摘要快照；除新闻雷达和交易复盘外，其余页面默认读取 snapshot。
- 侧边栏按“当前有变化/有重要事件”动态提示主题页面，低频且无变化的主题统一收纳。
- 数据库保留 SQLite 作为事实库；页面读取快照表，后台任务短事务写入，降低读写竞争。
- OKX 账户、订单、持仓和行情增加 WebSocket 接入，Redis 作为短期实时缓存，SQLite 作为可追溯事实记录，REST 作为启动校验和断线补偿。

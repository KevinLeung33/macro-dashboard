...
- 首页：宏观分析工作台入口。默认先看当前环境、跨资产温度计、数据质量、变化与催化剂，再进入研究假设或 Crypto 交易工作台。
...
首页只展示有限的状态摘要和变化，不为每张图单独生成结论，避免读者在几十条结论中失去重点。完整图表集中在各主题页的“详细数据”标签中；每个主题页还提供“事件与证据”入口。每个摘要都会尽量同时展示数据日期、来源和质量状态。

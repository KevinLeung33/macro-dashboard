...
enabled = st.toggle("启用紧急新闻推送", key="alert_enabled", on_change=_save_rule_changes)
c1, c2, c3, c4 = st.columns(4)
with c1:
    min_severity = st.selectbox("最低严重度", severity_options, key="alert_min_severity", on_change=_save_rule_changes)
with c2:
    min_confidence = st.slider("最低置信度", 0.5, 1.0, key="alert_min_confidence", step=0.05, on_change=_save_rule_changes)
with c3:
    min_articles = st.number_input("最少来源文章", min_value=1, max_value=10, key="alert_min_articles", step=1, on_change=_save_rule_changes)
with c4:
    max_age_minutes = st.selectbox("事件有效窗口", window_options, key="alert_max_age_minutes", format_func=lambda value: f"{value} 分钟", on_change=_save_rule_changes)

left, right = st.columns(2)
with left:
    event_types = st.multiselect("关注的关键事件", EVENT_TYPES, key="alert_event_types", on_change=_save_rule_changes)
with right:
    assets = st.multiselect("关注的关键资产", ASSETS, key="alert_assets", on_change=_save_rule_changes)
channels = st.multiselect("推送渠道", CHANNELS, key="alert_channels", on_change=_save_rule_changes)

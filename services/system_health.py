...
from db.repository import query_news_feed_states
...
def _send_transition(state, critical, warnings):
    if state == "ok":
        message = "宏观看板 P0/P1 健康检查已恢复正常。"
        notify(message, parse_notify_channels(), title="宏观看板健康恢复", level="info")
        return
    title = "宏观看板 P0/P1 严重告警" if state == "critical" else "宏观看板 P1 预警"
    lines = [title]
    if critical:
        lines.append("严重问题：")
